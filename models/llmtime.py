...
def generate_predictions(
    ...
    **kwargs
    ...
    if parallel and len(input_strs) > 1:
        ...
        # logger.debug(f"kharo olagh: {completion_fn}")
        # logger.debug(f"Completions list in generated_predictions: {completions_list}")
    def completion_to_pred(completion, inv_transform): 
        logger.debug(f"Completion before deserialization: {completion}")
...
def get_llmtime_predictions_data(train, test, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, **kwargs):
    ...
    median = kwargs.get('few_shot_median', None)
    if median:
        transformed_medians = [scaler.transform(median) for scaler in scalers]
        logger.debug(f"Transformed medians: {transformed_medians}")
        logger.debug(f"Transformed medians shape: {[scaled_median.shape for scaled_median in transformed_medians]}")
        append_to_extra_input = [serialize_arr(scaled_median.squeeze(), settings) for scaled_median in transformed_medians]
    else:
        append_to_extra_input = []
    
    #! For now let's stick to one input
    if (extra_input := kwargs.pop('extra_input', None)):
        logger.debug(f"Medians that are added to the extra input: {append_to_extra_input[0]}")
        few_shot_text = f"Future estimation without co-variates: The following sequence is a set of predicted values generated by another forecasting model. These values are not actual observations but reflect an estimation of the future behavior of the target sequence. You may consider this as an external opinion or auxiliary input to support your own prediction. The external opinion does not take the co-variates effect into account and that is your responsibility to factor in their effect. The number of steps you separate using '{settings.time_sep}' should be {test_len}.\n"
        extra_input = few_shot_text + append_to_extra_input[0] + extra_input if append_to_extra_input else extra_input
        completion_fn = partial(completion_fn, extra_input=extra_input)
    
    if (Co_variates := kwargs.pop('Co_variates', None)):
        assert isinstance(Co_variates, list)
        covariate_texts = kwargs.pop('Co_variate_texts', None)
        if covariate_texts is None:
            covariate_texts = ['This is a co-variate. It might help you predict the main time series by being correlated with it.']*len(Co_variates)
        assert isinstance(covariate_texts, list)

        final_covariate_text = f"Co-variates: The following are a set of known values known as co-variates for some additional variables that may help you predict the main time series. Each time step is separated using '{settings.time_sep}'.\n"
        for i in range(len(Co_variates)):
            covariate_str = serialize_arr(Co_variates[i], settings)
            if not covariate_str.endswith(settings.time_sep):
                covariate_str += settings.time_sep
            final_covariate_text += covariate_texts[i] + '\n' + covariate_str + '\n'
        completion_fn = partial(completion_fn, co_variates=final_covariate_text)
        logger.debug(f"Co-variate that is added to the extra input: {final_covariate_text}")
    ...
    if num_samples > 0:
        ...
        # logger.debug(f"Generated predictions: {preds}")
        # logger.debug(f"completion fns: {completion_fn}")
