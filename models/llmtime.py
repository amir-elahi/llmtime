...
def get_llmtime_predictions_data(train, test, model, settings, num_samples=10, temp=0.7, alpha=0.95, beta=0.3, basic=False, parallel=True, **kwargs):
    ...
    if (extra_input := kwargs.pop('extra_input', None)):
        ...
        few_shot_text = "The following sequence is a set of predicted values generated by another forecasting model. These values are not actual observations but reflect the model’s estimation of the future behavior of the time series. You may consider this as an external opinion or auxiliary input to support your own prediction."
        extra_input = few_shot_text + '\n' + append_to_extra_input[0] + '\n' + extra_input if append_to_extra_input else extra_input
